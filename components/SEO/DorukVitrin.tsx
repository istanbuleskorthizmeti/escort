    ...
    >
      ...
      <div className={host?.includes('dorukcanay.digital') ? "max-w-5xl mx-auto flex flex-col pt-4" : "max-w-xl mx-auto flex flex-col pt-4"}>
        ...
        <div className={host?.includes('dorukcanay.digital') ? "grid grid-cols-1 md:grid-cols-2 gap-6 w-full px-4" : "flex flex-col gap-8 w-full px-4"}>
          ...
          <div className={`relative min-h-[280px] bg-zinc-950/80 border-2 border-dashed border-rose-500/40 rounded-2xl overflow-hidden flex flex-col md:flex-row p-6 items-center justify-between gap-6 shadow-2xl hover:border-rose-500 transition-all duration-300 ${host?.includes('dorukcanay.digital') ? 'md:col-span-2' : ''}`}>
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_0%_50%,rgba(225,29,72,0.12),transparent)] pointer-events-none"></div>
            
            <div className="flex flex-col gap-2 relative z-10 text-center md:text-left flex-1">
              <span className="bg-rose-500/10 text-rose-400 border border-rose-500/30 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest w-fit mx-auto md:mx-0 animate-pulse">
                VİTRİN'E İLAN EKLEMEK İÇİN
              </span>
              <h4 className="text-white font-black text-lg sm:text-xl uppercase tracking-wider leading-none mt-1">
                Vitrin'e İlan Eklemek İçin
              </h4>
              <p className="text-zinc-400 text-xs max-w-lg leading-relaxed">
                Vitrin'e ilan eklemek için. İletişime Geçin!
              </p>
            </div>

            <div className="flex flex-col sm:flex-row items-center gap-6 relative z-10 w-full md:w-auto shrink-0">
              <div className="relative w-32 h-24 rounded-lg overflow-hidden border border-zinc-900 bg-zinc-900/40 shadow-lg hidden sm:flex items-center justify-center">
                <span className="text-zinc-600 hover:text-rose-500 text-3xl font-extralight transition-colors duration-500 select-none">
                  +
                </span>
              </div>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full sm:w-auto px-8 py-4 bg-gradient-to-tr from-rose-600 to-amber-500 text-white font-black uppercase text-[11px] tracking-widest rounded-xl hover:scale-105 transition-all text-center shadow-lg shadow-rose-600/30 flex items-center justify-center gap-2"
              >
