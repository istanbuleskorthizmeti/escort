...
async function run() {
  ...
  try {
    ...
    if (!targetPage.url().includes('search.google.com/search-console/sitemaps')) {
    ...
    // Close the feedback panel first if open
    ...
    if (await input.count() > 0) {
      ...
      if (await submitBtn.count() > 0) {
        ...
        await targetPage.screenshot({ path: path.join(process.cwd(), 'gsc-sitemaps-pw-success.png') });
        console.log("📸 Saved gsc-sitemaps-pw-success.png");
        
        const afterText = await targetPage.evaluate(() => document.body.innerText);
        fs.writeFileSync('gsc-sitemaps-pw-success.txt', afterText);
        ...
        if (await dismissBtn.count() > 0) {
          ...
          await targetPage.waitForTimeout(2000);
          await targetPage.screenshot({ path: path.join(process.cwd(), 'gsc-sitemaps-pw-final.png') });
        }
      } else {
