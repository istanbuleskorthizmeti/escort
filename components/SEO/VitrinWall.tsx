      ...
      <div className="px-10 mb-12 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="text-center md:text-left">
          <h2 className="text-4xl md:text-5xl font-black italic text-white uppercase tracking-tighter mb-3 leading-none">
            DRKCNAY <span className="text-(--brand-color)">ELITE</span> <span className="text-zinc-700">VİTRİN</span>
          </h2>
      ...
      <div className={containerClass}>
        {shuffledIndices.map((originalIndex, index) => {
          ...
          return (
            <div 
              key={index} 
              className="relative group border border-zinc-900 rounded-dynamic overflow-hidden bg-black aspect-[3/4] shadow-2xl transition-all duration-1000 hover:-translate-y-4 hover:shadow-(--brand-color)/20 hover:border-(--brand-color)/30"
            >
              {/* Luxury Hover Overlay */}
              <div className="absolute inset-0 bg-(--brand-color)/10 opacity-0 group-hover:opacity-100 transition-opacity duration-1000 z-5"></div>
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 z-30 flex flex-col items-center transform translate-y-4 group-hover:translate-y-0 transition-transform duration-700">
                ...
                <div className="h-0 group-hover:h-12 opacity-0 group-hover:opacity-100 transition-all duration-700 w-full flex justify-center overflow-hidden">
                    <a 
                      href={REDIRECT_URL}
                      className="pointer-events-auto bg-white text-black px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-(--brand-color) hover:text-white transition-all"
                    >
