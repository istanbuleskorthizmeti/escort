...
const rawDir = 'C:\\Users\\onurk\\Desktop\\vitrin dorukcan\\Ceren';
const localMediaDir = path.join(process.cwd(), 'public', '_media', 'vitrin');

const files = [
  'ceren-escort-drkcnay (1).jpeg',
  'ceren-escort-drkcnay (2).jpeg',
  'ceren-escort-drkcnay (3).jpeg',
  'ceren-escort-drkcnay (4).jpeg'
];

async function run() {
  try {
    console.log('🎨 [SHARP] Processing Ceren profile images...');
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const destName = `istanbul-kaporasiz-escort-ceren-${i + 1}.webp`;
    ...
    console.log('🏁 [SUCCESS] All Ceren profile images are optimized and deployed!');
    ssh.dispose();
  } catch (err) {
