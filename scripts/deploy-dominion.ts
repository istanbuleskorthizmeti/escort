...
async function deployDominion() {
  try {
    ...
    console.log('🧊 [SYNC] Syncing build assets to Nginx static root /var/www/escortvip/.next/...');
    const syncResult = await ssh.execCommand('rsync -a --delete /root/esc/.next/ /var/www/escortvip/.next/');
    if(syncResult.stderr) console.warn('Sync warning/error:', syncResult.stderr);
