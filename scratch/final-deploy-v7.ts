...
function getAllFiles(dirPath: string, arrayOfFiles: string[] = []) {
    ...
    files.forEach(function(file) {
        if (fs.statSync(dirPath + "/" + file).isDirectory()) {
            ...
            // SKIP BINARY FILES to prevent corruption via cat >
...
async function deploy() {
    ...
    try {
        ...
        console.log('📦 Syncing ALL lib/, components/, and app/ (Text Files Only) ...');
        ...
        console.log(`✅ Total Text Files Uploaded: ${allFiles.length}`);

        console.log('🚀 Starting ULTIMATE Remote Build (V7)...');
