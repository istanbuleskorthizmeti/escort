...
async function run() {
  ...
  try {
    ...
    await targetPage.screenshot({ path: path.join(process.cwd(), 'readme-git-conn.png') });
    console.log("Screenshot saved: readme-git-conn.png");

  } catch (err: any) {
    console.error("Error navigating to Git connection page:", err.message);
  } finally {
    await browser.close();
  }
}
