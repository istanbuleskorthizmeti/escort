...
async function run() {
  ...
  // 1. Create a page to request the login link and do the setup
  const actionPage = await context.newPage();

  try {
    console.log("🧼 Logging out of current session on dash.readme.com first...");
    ...
    console.log("⏳ Waiting 15 seconds for magic link email to arrive in Gmail...");
    await actionPage.waitForTimeout(15000);

    // 2. Find or open the Gmail page
    ...
    console.log("🔍 Locating ReadMe login link email...");
    ...
    console.log("📬 Email clicked. Waiting 5 seconds for content to load...");
    ...
    // 3. Navigate to istanbul-escort integrations
