...
async function run() {
  ...
  try {
    ...
    // Reload the page to clear any sidebar overlays
    console.log("Reloading page for a clean slate...");
    ...
    if (await input.count() > 0) {
      console.log("Input found. Setting value programmatically and dispatching events...");
      
      // Focus the input first
      await input.focus();
      await page.waitForTimeout(500);

      // Programmatically set value and dispatch input/change events to sync Angular model
      await page.evaluate(() => {
      ...
      // Now click and type one character and delete it, to trigger physical keyboard bindings
      await input.click();
      ...
      if (await submitBtn.count() > 0) {
        ...
        await page.screenshot({ path: path.join(process.cwd(), 'gsc-sitemaps-root-final-submit-success.png') });
        console.log("📸 Saved gsc-sitemaps-root-final-submit-success.png");
        ...
        if (await dismissBtn.count() > 0) {
          ...
          await page.screenshot({ path: path.join(process.cwd(), 'gsc-sitemaps-root-final-submit-final.png') });
        }
      } else {
