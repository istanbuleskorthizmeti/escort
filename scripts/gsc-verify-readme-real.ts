...
async function run() {
  ...
  try {
    const siteUrl = "https://search.google.com/search-console/not-verified?original_url=/search-console/users?resource_id%3Dhttps://istanbul-escort.readme.io/&original_resource_id=https://istanbul-escort.readme.io/";
    console.log(`Navigating GSC to: ${siteUrl}`);
    await page.goto(siteUrl, { waitUntil: 'load', timeout: 45000 });
    await page.waitForTimeout(6000);

    await page.screenshot({ path: path.join(process.cwd(), 'gsc-verify-before-real.png') });
    console.log("Screenshot saved: gsc-verify-before-real.png");
    ...
    if (clickResult.success) {
      ...
      await page.screenshot({ path: path.join(process.cwd(), 'gsc-verify-after-real.png') });
      console.log("Screenshot saved: gsc-verify-after-real.png");
