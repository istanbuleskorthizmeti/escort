...
async function main() {
  ...
  try {
    ...
    console.log('🚀 [2/7] Uploading bundle using legacy SCP...');
    const scpCmd = `scp -O -P ${remotePort} -o StrictHostKeyChecking=no ${tarFile} root@${remoteHost}:/root/${tarFile}`;
    ...
    for (const cmd of remoteCmds) {
      ...
      // Run command via ssh command line
